...
import { BookOpen, Lock, Settings, Video, Globe, ChevronDown, Send, Shield, FileText, UserCheck, AlertTriangle, CheckCircle, Clock, MapPin, Mail, ExternalLink, Stethoscope, Zap, BarChart2, ClipboardList } from "lucide-react";
...
const C = {
  ...
  peach:"#EBA89B", lightPeach:"#F5D3CD", peachXLight:"rgba(235,168,155,0.12)",
...
function WheelMark({ size=24 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {[45,135,225,315].map(r=>spoke(r,C.lavender))}
      {[0,90].map(r=>spoke(r,C.orange))}
    </svg>
  );
}
  ...
  return (
    <div style={{marginBottom:20}}>
      <h2 style={{fontFamily:"Montserrat,sans-serif", fontWeight:700, fontSize:20,
        color:C.charcoal, marginBottom:10}}>{children}</h2>
...
function HoverCard({ children, style={} }) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{transition:"all 0.18s", transform:hov?"translateY(-2px)":"none",
  ...
  return (
    <button onClick={onClick}
      ...
      style={{padding:"13px 18px", whiteSpace:"nowrap",
        ...
        background:active?C.lavender:hov?"rgba(177,165,247,0.45)":"transparent",
...
function CollapsibleSection({ title, summary, children, variant="light" }) {
  ...
  const variants = {
    light:    { header:C.white,   titleColor:C.charcoal, summaryColor:C.taupe,   body:C.white,   border:open?"rgba(57,75,153,0.25)":"rgba(15,27,31,0.08)",  chevBg:open?C.indigo:"rgba(177,165,247,0.15)", chevColor:open?C.white:C.lavender },
    indigo:   { header:`linear-gradient(135deg,${C.indigo},#2d3d85)`, titleColor:C.white, summaryColor:"rgba(255,255,255,0.6)", body:`linear-gradient(135deg,${C.indigo},#2d3d85)`, border:open?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.08)", chevBg:open?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.1)", chevColor:open?C.white:"rgba(255,255,255,0.6)" },
    forest:   { header:`linear-gradient(135deg,${C.forest},#1d4038)`, titleColor:C.white, summaryColor:"rgba(255,255,255,0.6)", body:`linear-gradient(135deg,${C.forest},#1d4038)`,  border:open?"rgba(255,255,255,0.15)":"rgba(255,255,255,0.06)", chevBg:open?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.1)", chevColor:open?C.white:"rgba(255,255,255,0.6)" },
    mint:     { header:C.lightMint, titleColor:C.forest, summaryColor:C.forest,  body:C.lightMint, border:open?`rgba(38,84,79,0.3)`:`rgba(38,84,79,0.12)`,   chevBg:open?C.forest:"rgba(38,84,79,0.12)", chevColor:open?C.white:C.forest },
    lavender: { header:C.lightLavender, titleColor:C.indigo, summaryColor:C.indigo, body:C.lightLavender, border:open?`rgba(57,75,153,0.3)`:`rgba(57,75,153,0.15)`, chevBg:open?C.indigo:"rgba(57,75,153,0.1)", chevColor:open?C.white:C.indigo },
  };
  const v = variants[variant] || variants.light;
  return (
    <div style={{borderRadius:16, overflow:"hidden", border:`1px solid ${v.border}`,
      boxShadow:open?"0 4px 20px rgba(15,27,31,0.1)":"0 1px 4px rgba(15,27,31,0.05)", transition:"all 0.2s"}}>
      <div onClick={()=>setOpen(o=>!o)}
        style={{background:v.header, padding:"20px 24px", cursor:"pointer",
          display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16}}>
        <div style={{flex:1}}>
          <div style={{fontFamily:"Montserrat,sans-serif", fontWeight:700, fontSize:16, color:v.titleColor, marginBottom:summary?6:0}}>{title}</div>
          {summary && <p style={{fontSize:13, color:v.summaryColor, lineHeight:1.6, maxWidth:520}}>{summary}</p>}
        </div>
        <div style={{width:28, height:28, borderRadius:"50%", flexShrink:0, marginTop:2,
          background:v.chevBg, display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.2s"}}>
          <ChevronDown size={13} color={v.chevColor}
            style={{transform:open?"rotate(180deg)":"none", transition:"transform 0.2s"}}/>
        </div>
      </div>
      <div style={{maxHeight:open?"900px":0, overflow:"hidden", transition:"max-height 0.4s ease", background:v.body}}>
...
function fireConfetti(canvas) {
  ...
  const particles=Array.from({length:80},()=>({
    x:canvas.width/2+(Math.random()-0.5)*80, y:0,
  ...
  const draw=()=>{
    ...
    let alive=false;
    particles.forEach(p=>{
      p.x+=p.vx; p.y+=p.vy; p.vy+=0.12; p.rot+=p.vr; p.alpha-=0.012;
...
function SetupScreen({ onComplete }) {
  ...
  const inputStyle = { width:"100%", border:`1px solid rgba(57,75,153,0.2)`, borderRadius:10, padding:"11px 14px", fontSize:14, fontFamily:"Inter,sans-serif", color:C.charcoal, background:C.white, outline:"none", boxShadow:"0 1px 3px rgba(57,75,153,0.06)" };
  ...
  return (
    <div style={{minHeight:"100vh", background:`linear-gradient(160deg, #ffffff 0%, #f8f7ff 50%, #f0faf0 100%)`, display:"flex", alignItems:"center", justifyContent:"center", padding:24}}>
      <div className="fadeUp" style={{background:C.white, borderRadius:24, padding:"40px 36px", maxWidth:440, width:"100%", boxShadow:"0 16px 48px rgba(57,75,153,0.15)", position:"relative", overflow:"hidden", border:`1px solid rgba(57,75,153,0.1)`}}>
        ...
        <div style={{position:"absolute",top:60,right:-12,width:30,height:30,background:"rgba(177,165,247,0.12)",transform:"rotate(45deg)"}}/>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:28,position:"relative"}}>
          <WheelMark size={28}/>
          <div>
            <div style={{fontFamily:"Montserrat,sans-serif",fontWeight:800,fontSize:16,color:C.charcoal,letterSpacing:"-0.3px"}}>Cartwheel</div>
        ...
        <div style={{position:"relative"}}>
          ...
          <p style={{fontSize:14,color:C.taupe,lineHeight:1.65,marginBottom:28}}>Let's personalize your onboarding hub so everything feels made for you.</p>
...
const CHECKLIST_GROUPS = [
  { key:"72hrs",       label:"Complete Within 72 Hours",       accent:C.brick,  color:"#fce9ee", Icon:AlertTriangle, note:"Delays here will push your start date.",
    tasks:[{id:"c1",text:"Complete your Credentialing Survey",note:"Check email from Credentialing@cartwheelcare.org — use a computer, not mobile"},{id:"c2",text:"Set up your Rippling account",note:"Check spam/junk folder for the invitation email"}]},
  { key:"2weeks",      label:"Complete 2 Weeks Before Start",  accent:C.bronze, color:C.lightBronze, Icon:Clock, note:"Required to confirm your start date.",
    tasks:[{id:"c3",text:"Submit your Clinical Schedule form",note:"You'll be providing 10 clinical hours per week"},{id:"c4",text:"Complete State Compliance requirements",note:"Check if your state requires fingerprints or certifications"},{id:"c5",text:"Complete Cultural Competency Training",note:"Required for Cartwheel compliance standards"}]},
  { key:"beforestart", label:"Complete Before Your Start Date", accent:C.indigo, color:C.lavXLight, Icon:FileText,
    tasks:[{id:"c6",text:"Complete I-9 verification (W2 employees only)",note:"Full-time and MA part-time employees only"},{id:"c7",text:"Complete background check via Checkr",note:"Link sent after Rippling setup — expires in 7 days"},{id:"c8",text:"Set up direct deposit & tax withholding in Rippling"},{id:"c9",text:"Confirm your personal details in Rippling (legal name, address)"}]},
  { key:"day1",        label:"Day 1 Priorities",                accent:C.forest, color:C.mintXLight, Icon:CheckCircle,
    tasks:[{id:"c10",text:"Log in — Orientation at 11am EST / 10am CST / 9am MST / 8am PST"},{id:"c11",text:"Attend New Employee Orientation (recorded if you can't make it live)"},{id:"c12",text:"Complete Rippling trainings: HIPAA, Cybersecurity, Harassment"},{id:"c13",text:"Connect with your Clinical Onboarding contact"}]},
];
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Sticky header */}
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(255,255,255,0.95)",backdropFilter:"blur(8px)",borderBottom:"1px solid rgba(57,75,153,0.1)",padding:"12px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <WheelMark size={22}/>
          <span style={{fontFamily:"Montserrat,sans-serif",fontWeight:800,fontSize:15,color:C.charcoal,letterSpacing:"-0.3px"}}>Cartwheel</span>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{background:C.lavXLight,borderRadius:99,padding:"4px 12px",border:`1px solid rgba(57,75,153,0.12)`}}>
            <span style={{fontSize:12,color:C.indigo,fontFamily:"Montserrat,sans-serif",fontWeight:600}}>
              ...
              {user.startDate && <span style={{opacity:0.7}}> · Starts {new Date(user.startDate).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</span>}
            </span>
          </div>
      ...
      <div style={{background:`linear-gradient(160deg, #ffffff 0%, #f8f7ff 50%, #f0faf0 100%)`,padding:"52px 28px 0",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:30,right:260,width:20,height:20,background:"rgba(57,75,153,0.1)",transform:"rotate(30deg)",animation:"floatB 6s ease-in-out infinite"}}/>
        ...
        <div style={{maxWidth:820,margin:"0 auto"}}>
          <FadeIn delay={0}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:C.white,borderRadius:99,padding:"6px 14px",marginBottom:20,boxShadow:"0 2px 8px rgba(57,75,153,0.1)",border:`1px solid rgba(57,75,153,0.1)`}}>
              ...
              <span style={{fontFamily:"Montserrat,sans-serif",fontSize:11,fontWeight:600,color:C.charcoal,letterSpacing:"0.05em"}}>{user.role} · Serving {user.state}</span>
            </div>
          </FadeIn>
          ...
          <FadeIn delay={200}>
            <div style={{background:C.lightBrick,borderRadius:14,padding:"14px 18px",marginBottom:28,border:`1px solid rgba(92,30,55,0.2)`,display:"flex",gap:12,alignItems:"flex-start",maxWidth:560}}>
              <AlertTriangle size={16} color={C.brick} style={{flexShrink:0,marginTop:2}}/>
              <p style={{fontSize:13,color:C.brick,lineHeight:1.65}}>
          ...
          <FadeIn delay={260}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,
              background:`linear-gradient(135deg,${C.sky} 0%,#9ec5d8 50%,${C.lightMint} 100%)`,
              borderRadius:"14px 14px 0 0",overflow:"hidden",
              boxShadow:"0 4px 20px rgba(57,75,153,0.12)"}}>
              {[["350+","School Districts","Across the US"],["1.5M","Students","In our care network"],["10hrs","Per Week","Clinical commitment"],["7–10","Days","Avg. student wait time"]].map(([n,l,s],i)=>(
                <div key={i} style={{padding:"18px 16px",
                  ...
                  <div style={{fontFamily:"Montserrat,sans-serif",fontWeight:700,fontSize:11,color:C.indigo,marginTop:2,opacity:0.85}}>{l}</div>
                  <div style={{fontSize:11,color:C.indigo,marginTop:2,opacity:0.5}}>{s}</div>
                </div>
              ))}
            </div>
          </FadeIn>

          {/* Tabs — light lavender bg, indigo font, lavender hover/active */}
          <FadeIn delay={300}>
            <div style={{background:C.lightLavender,borderRadius:"0 0 14px 14px",display:"flex",overflowX:"auto"}}>
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"36px 28px 64px"}}>
        {tab==="home"          && <HomeTab checks={checks} toggle={toggle} pct={pct} done={done} setTab={setTab} user={user}/>}
...
// ── HOME ──────────────────────────────────────────────────────────────────────
function HomeTab({ checks, toggle, pct, done, setTab, user }) {
  const firstName = user.name.split(" ")[0];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:22}}>
      <FadeIn delay={0}>
        <div style={{background:`linear-gradient(135deg,${C.indigo},#2d3d85)`,borderRadius:20,padding:"26px 28px",position:"relative",overflow:"hidden",boxShadow:"0 8px 32px rgba(57,75,153,0.22)"}}>
          ...
          <p style={{fontSize:13,color:"rgba(255,255,255,0.65)",lineHeight:1.75}}>
            {user.startDate && <span>Your start date is <strong style={{color:C.mint}}>{new Date(user.startDate).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</strong>. </span>}
            Bookmark this page — it's your single source of truth through onboarding. Everything from credentialing to Day 1 logistics lives right here.
          </p>
        </div>
      </FadeIn>
        ...
        <div style={{background:C.brick,borderRadius:16,padding:"20px 22px",boxShadow:"0 4px 16px rgba(92,30,55,0.18)"}}>
          ...
          {urgentItems.map((item,i)=>(
            <div key={i} onClick={()=>toggle(item.id)}
              ...
              <div style={{width:20,height:20,borderRadius:5,flexShrink:0,marginTop:1,border:`2px solid ${checks[item.id]?"rgba(255,255,255,0.6)":"rgba(255,255,255,0.3)"}`,background:checks[item.id]?"rgba(255,255,255,0.25)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>
          ...
          {journey.map(s=>{
            ...
            return (
              <div key={s.key} onClick={()=>setOpenJ(isOpen?null:s.key)}
                style={{background:isOpen?C.lavXLight:C.lightSky,borderRadius:16,padding:"18px 20px",border:`1px solid ${isOpen?"rgba(57,75,153,0.2)":C.sky}`,cursor:"pointer",transition:"all 0.2s",boxShadow:isOpen?"0 4px 16px rgba(57,75,153,0.1)":"0 1px 4px rgba(15,27,31,0.05)"}}>
      ...
      <FadeIn delay={140}>
        <div style={{background:C.lightMint,borderRadius:16,padding:"20px 24px",border:`1px solid rgba(38,84,79,0.15)`,borderLeft:`3px solid ${C.forest}`,boxShadow:"0 1px 4px rgba(38,84,79,0.06)"}}>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <FadeIn delay={0}>
        ...
        <p style={{fontSize:14,color:"#4a5568",lineHeight:1.75,marginBottom:20}}>
          Each phase has a deadline that affects your start date. Complete in order — don't let the credentialing survey slip past 72 hours.
        </p>
        <div style={{background:`linear-gradient(135deg,${C.charcoal},#1a2e35)`,borderRadius:16,padding:"20px 24px",display:"flex",gap:20,alignItems:"center",marginBottom:20,position:"relative",overflow:"hidden",boxShadow:"0 4px 16px rgba(15,27,31,0.2)"}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{background:"rgba(177,165,247,0.15)",borderRadius:99,height:6}}>
            ...
            {pct===100 && <div className="fadeUp" style={{fontSize:12,fontWeight:600,color:C.mint,marginTop:6}}>You're fully onboarded. Welcome to the team!</div>}
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {CHECKLIST_GROUPS.map(group=>{
            ...
            return (
              <div key={group.key} style={{background:isComplete?C.mintXLight:C.white,border:`1px solid ${isOpen?`${group.accent}55`:isComplete?"rgba(38,84,79,0.2)":"rgba(15,27,31,0.08)"}`,borderRadius:14,overflow:"hidden",boxShadow:"0 1px 4px rgba(15,27,31,0.04)"}}>
                <button onClick={()=>setOpen(isOpen?null:group.key)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"none"}}>
                  <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,background:isOpen?group.accent:isComplete?"rgba(38,84,79,0.12)":"rgba(15,27,31,0.06)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {isComplete ? <svg width={12} height={10} viewBox="0 0 12 10"><polyline points="1,5 5,9 11,1" fill="none" stroke={isOpen?C.white:C.forest} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:22}}>
        ...
        <div style={{background:C.brick,borderRadius:16,padding:"22px 24px",boxShadow:"0 4px 16px rgba(92,30,55,0.18)"}}>
          <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div>
              ...
              <p style={{fontSize:13,color:"rgba(255,255,255,0.75)",lineHeight:1.7}}>If your credentialing survey isn't completed within 72 hours, your start date will be pushed. The earlier you submit, the faster we complete your payor enrollment.</p>
            </div>
          </div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={100}>
        ...
        <div style={{background:C.lightLavender,borderRadius:14,padding:"18px 20px",border:`1px solid rgba(57,75,153,0.15)`,marginBottom:10}}>
          ...
          <p style={{fontSize:13,color:"#4a5568",lineHeight:1.7}}>Some states require fingerprints or certifications for clinicians who see patients there. Check the <a href="https://sites.google.com/cartwheelcare.org/cartwheelclinicalonboarding/home" target="_blank" style={{color:C.indigo,fontWeight:600}}>Clinical Onboarding Website</a> for your state's specific requirements.</p>
        </div>
        <div style={{background:C.lightMint,borderRadius:14,padding:"14px 18px",border:`1px solid rgba(38,84,79,0.15)`}}>
          <p style={{fontSize:13,color:C.forest,lineHeight:1.7}}>Completed a state requirement before? Email <a href="mailto:compliance@cartwheelcare.org" style={{color:C.forest,fontWeight:700,textDecoration:"underline"}}>compliance@cartwheelcare.org</a> to verify if it's still valid.</p>
        </div>
      </FadeIn>
      <FadeIn delay={140}>
        ...
        <div style={{background:C.lavXLight,borderRadius:14,padding:"18px 20px",border:`1px solid rgba(57,75,153,0.12)`}}>
          ...
          <a href="https://drive.google.com/file/d/1en33We1N_XCOFIhMv7gfe_DNa4nYscH2/view" target="_blank"
            style={{display:"inline-flex",alignItems:"center",gap:8,background:C.indigo,color:C.white,borderRadius:10,padding:"10px 18px",fontSize:13,fontWeight:700,fontFamily:"Montserrat,sans-serif",textDecoration:"none",boxShadow:"0 4px 12px rgba(57,75,153,0.25)"}}>
      ...
      <FadeIn delay={170}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            ...
            {title:"State compliance & I-9",  bg:C.mintXLight,    ac:C.forest, email:"clinical.onboarding@cartwheelcare.org",     desc:"Fingerprints, certifications, W2 I-9 requirements"},
          ].map((c,i)=>(
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:26}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {tools.map((t,i)=>(
            <HoverCard key={i} style={{background:C.lightSky,borderRadius:14,padding:"18px",border:`1px solid ${C.sky}`}}>
              ...
              {t.link && <a href={t.link} target="_blank" style={{fontSize:12,color:C.indigo,fontWeight:700,display:"flex",alignItems:"center",gap:4}}><ExternalLink size={11}/> Open</a>}
            </HoverCard>
          ))}
        </div>
      </FadeIn>
      <FadeIn delay={60}>
        ...
        <div style={{background:C.white,borderRadius:16,overflow:"hidden",border:"1px solid rgba(15,27,31,0.08)",boxShadow:"0 1px 4px rgba(15,27,31,0.05)"}}>
          {[
        ...
        <CollapsibleSection title="What We Do" variant="lavender" summary="Cartwheel partners directly with K-12 districts to make quality mental health care accessible when students need it.">
          <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:10}}>
            {[
              ...
              <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",background:C.white,borderRadius:12,padding:"14px 16px",border:`1px solid rgba(57,75,153,0.1)`,boxShadow:"0 1px 4px rgba(57,75,153,0.06)"}}>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:22}}>
      <FadeIn delay={0}>
        ...
        <div style={{background:`linear-gradient(135deg,${C.indigo},#2d3d85)`,borderRadius:16,
          padding:"32px",textAlign:"center",boxShadow:"0 8px 28px rgba(57,75,153,0.2)"}}>
          <div style={{width:56,height:56,borderRadius:"50%",background:"rgba(177,165,247,0.2)",
          ...
          <div style={{fontFamily:"Montserrat,sans-serif",fontWeight:800,fontSize:20,
            color:C.indigo,background:C.white,display:"inline-block",
            borderRadius:10,padding:"4px 20px",marginBottom:12}}>Coming Soon</div>
          <p style={{fontSize:14,color:"rgba(255,255,255,0.65)",lineHeight:1.75,maxWidth:400,margin:"0 auto"}}>
            This section will cover your clinical toolstack — EHR access, telehealth platform setup, scheduling, and session documentation guides.
          </p>
        </div>
      </FadeIn>
      <FadeIn delay={60}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
          {[
            ...
            <div key={i} style={{background:`linear-gradient(135deg,${C.white} 0%,#f8f7ff 100%)`,
              borderRadius:12,padding:"18px 16px",
              border:`1px solid rgba(57,75,153,0.1)`,textAlign:"center",
              boxShadow:"0 1px 4px rgba(57,75,153,0.06)"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {SUGGESTIONS.map((s,i)=>(
                  <button key={i} onClick={()=>send(s)}
                    style={{background:C.white,border:`1px solid rgba(57,75,153,0.12)`,borderRadius:99,padding:"9px 16px",fontSize:13,color:C.charcoal,textAlign:"left",boxShadow:"0 1px 4px rgba(57,75,153,0.06)",transition:"all 0.15s",fontFamily:"inherit"}}
